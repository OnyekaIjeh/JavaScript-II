...
function runnerHasLargeShirt(runner) {
  return runner.shirt_size === "L";
}
let runnersLargeSizeShirt = runners.filter(runnerHasLargeShirt);
...
// Problem 1
let secondFullNames = runners.reduce((acc, runner) => {
  acc.push(`${runner.first_name} ${runner.last_name}`);
  return acc;
}, []);
console.log(secondFullNames);

// Problem 2
function runnerDonationGreaterThan200(runner) {
  return runner.donation > 200;
}

let runnersWithDonationGreaterThan200 = runners.filter(
  runnerDonationGreaterThan200
);
console.log(runnersWithDonationGreaterThan200);

// Problem 3
function donationTotalReducer(total, runner) {
  return total + runner.donation;
}

function runnerHasSmallShirt(runner) {
  return runner.shirt_size === "S";
}

let totalDonationsOfRunnersWithSmallShirts = runners
  .filter(runnerHasSmallShirt)
  .reduce(donationTotalReducer, 0);
console.log(totalDonationsOfRunnersWithSmallShirts);
